...
class PanopticForm extends StatefulWidget {
  ...
  /// from disposed [PanopticFormField]s. This is useful for dynamic forms where
  ...
  /// A list of form fields to be displayed in the form. This is an alternative to using [child].
  /// If both are provided, [child] will be used.
  /// Using this property will display the fields within a [PanopticCard] automatically.
  final List<Widget>? formFields;

  final PanopticButton? submitButton;
  final PanopticIconButton? deleteButton;
  ...
  static PanopticFormState? of(BuildContext context) =>
      context.findAncestorStateOfType<PanopticFormState>();

  @override
  PanopticFormState createState() => PanopticFormState();
}

/// A type alias for a map of form fields.
typedef PanopticFormFields
    = Map<String, PanopticFormFieldState<PanopticFormField<dynamic>, dynamic>>;

class PanopticFormState extends State<PanopticForm> {
  final PanopticFormFields _fields = {};
  ...
  PanopticFormFields get fields => _fields;
  ...
  void registerField(String name, PanopticFormFieldState field) {
  ...
  void unregisterField(String name, PanopticFormFieldState field) {
    assert(
  ...
  Widget build(BuildContext context) {
    assert(() {
      if (widget.child == null && widget.formFields == null) {
        throw FlutterError(
            'PanopticForm: You must provide a child or formFields.');
      }
      return true;
    }());
    return Form(
      ...
      child: _PanopticFormScope(
        ...
        child: FocusTraversalGroup(
          ...
          child: Column(
            children: [
              widget.child ??
                  PanopticCardColumn(children: widget.formFields ?? []),
              _buildButtonRow(),
            ],
          ),
        ),
  ...
  Widget _buildButtonRow() {
    return PanopticRow(
      mainAxisAlignment:
          (widget.submitButton != null && widget.deleteButton != null)
              ? MainAxisAlignment.spaceAround
              : MainAxisAlignment.end,
      children: [
        if (widget.submitButton != null && widget.deleteButton != null) ...{
          Expanded(
            child: widget.submitButton!,
          ),
          widget.deleteButton!,
        } else if (widget.submitButton != null) ...{
          widget.submitButton!,
...
class _PanopticFormScope extends InheritedWidget {
  const _PanopticFormScope({
    required super.child,
    required PanopticFormState formState,
  }) : _formState = formState;

  final PanopticFormState _formState;

  /// The [Form] associated with this widget.
  PanopticForm get form => _formState.widget;

  @override
  bool updateShouldNotify(_PanopticFormScope oldWidget) =>
